        ...
        <div className="w-full md:w-80 border-b md:border-b-0 md:border-r border-slate-100 bg-slate-50 flex flex-col h-full">
            <div className="p-6 border-b border-slate-200/50">
                ...
                <div className="relative">
                    ...
                    <input type="text" placeholder="Search chats..." className="w-full pl-10 pr-4 py-2 bg-white rounded-xl border border-slate-200 text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-100" />
                </div>
            </div>
            <div className="flex-1 overflow-y-auto">
                <div className="p-4 bg-white border-l-4 border-blue-600 cursor-pointer transition-colors shadow-sm">
                    <div className="flex justify-between items-start mb-1">
                        <span className="font-bold text-slate-900 text-sm">Codexier Support</span>
                        <span className="text-[10px] text-slate-400">now</span>
                    </div>
                    <p className="text-xs text-slate-500 truncate mb-1">{messages[messages.length - 1]?.text || 'Start a conversation'}</p>
        ...
        <div className="flex-1 flex flex-col h-full bg-slate-50/30">
            <div className="p-4 md:p-6 border-b border-slate-100 flex justify-between items-center bg-white shadow-sm z-10">
                <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full bg-slate-900 flex items-center justify-center text-white font-bold text-sm">CS</div>
                    <div>
            ...
            <div className="flex-1 p-4 md:p-6 overflow-y-auto space-y-6">
                 {/* Messages from Props */}
                 {messages.map((msg) => (
                    <div key={msg.id} className={`flex gap-4 ${msg.sender === 'client' ? 'flex-row-reverse' : ''}`}>
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white font-bold text-xs shrink-0 ${msg.sender === 'client' ? 'bg-slate-900' : 'bg-blue-600'}`}>
                            {msg.sender === 'client' ? 'ME' : 'CS'}
                        </div>
                        <div className="space-y-1 max-w-[80%]">
                            ...
                            <div className={`text-[10px] text-slate-400 ${msg.sender === 'client' ? 'text-right pr-2' : 'pl-2'}`}>
            ...
            <div className="p-4 bg-white border-t border-slate-100">
                <div className="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-xl p-2 pr-4 focus-within:ring-2 focus-within:ring-blue-100 transition-all">
                    <button className="p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors"><Paperclip size={20} /></button>
                    <input 
                        type="text" 
                    ...
                    />
                    <button onClick={handleSendChat} className="p-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors shadow-sm"><Send size={18} /></button>
                </div>
            </div>
        </div>
    </div>
                             ...
                             {STAGES.map((stage, idx) => {
                                 ...
                                 return (
                                     <div key={stage.id} className="flex flex-col items-center text-center">
                                         <div className={`w-8 h-8 rounded-full flex items-center justify-center mb-3 transition-all duration-500 z-10 ${
                                             isCompleted ? 'bg-slate-900 text-white shadow-lg' : 'bg-slate-100 text-slate-400'
                                         }`}>
