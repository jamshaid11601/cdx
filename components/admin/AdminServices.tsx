...
import { Plus, Search, Edit2, Trash2, X, Check, Image as ImageIcon, Sparkles } from 'lucide-react';
...
const AdminServices: React.FC<AdminServicesProps> = ({ gigs, setGigs }) => {
  ...
  const toggleStatus = (id: string) => {
    setGigs(gigs.map(g => g.id === id ? { ...g, status: g.status === 'active' ? 'inactive' : 'active' } : g));
  };
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
           ...
           <div className="relative w-full max-w-2xl bg-white rounded-[2rem] shadow-2xl max-h-[90vh] overflow-y-auto">
              ...
              <div className="p-8 border-t border-slate-100 bg-slate-50 flex justify-end gap-4 rounded-b-[2rem]">
                  ...
                  <button onClick={handleSave} className="px-8 py-3 bg-slate-900 text-white rounded-xl font-bold hover:bg-blue-600 shadow-lg shadow-blue-900/10 transition-all">
                      Save Changes
                  </button>
              </div>
           </div>
